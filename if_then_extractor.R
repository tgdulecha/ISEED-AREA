...
list_passive_cause_verbs <- c(
  paste(list_cause_verbs[grepl("ed$", list_cause_verbs)], "by", sep = " "),
  ...
  if (has_operator) {
    ...
    for (i in which(has_operator)) {
      if (grepl(
        ...
        operators[[i]] <- tolower(regmatches(
          x[[i]],
          positions[[i]]
        )[[1]])
...
relation_extractor <- function(text,
    ...
    for (i in which(has_operator)) {
      tryCatch({
        ...
        if (grepl(that_passive_end_regex_,
          ...
          end_form[[i]] <- gsub(paste("^([^,:!?;]){1,}([,:!?;][[:space:]]){1,}(?=.*( that | which |[,]{1}which ){1}.*(", all_at_end, "){1}.*)", sep = ""), "", y[[i]],
            perl = T,
            ignore.case = T
          )
          # end form pruning
          end_form[[i]] <- gsub(paste("(.*( that | which |[,]{1}which ){1}.*(", all_at_end, "){1}.*)\\K([,:!?;][[:space:]]){1,}([^,:!?;]){1,}$", sep = ""), "", end_form[[i]],
            perl = T,
            ignore.case = T
          )
          ...
          if (length(end_form[[i]]) > 0) {
            ...
            if (length(end_form[[i]]) > 1 && sum(contained_end_form[[i]]) >= 1) {
              ...
              y[[i]] <- paste(end_form[[i]][contained_end_form[[i]]], sep = "", collapse = "")
            } else {
              y[[i]] <- ""
            }
          } else {
    ...
    if (exists("causal_data")) {
      # print(causal_data)
      id <- causal_data[[1]]$id
      cause <- causal_data[[1]]$cause
      rel_negation <- causal_data[[1]]$rel_negation
      rel_operator <- causal_data[[1]]$rel_operator
      rel_passive_form <- causal_data[[1]]$rel_passive_form
      rel_creation <- causal_data[[1]]$rel_creation
      rel_destruction <- causal_data[[1]]$rel_destruction
      rel_causation <- causal_data[[1]]$rel_causation
      rel_coref_res <- causal_data[[1]]$rel_coref_res
      effect <- causal_data[[1]]$effect
      # print(ifres)
      # print(thenres)
      # return(list(causal_data,ifres,thenres))
      ...
      rel_negation <- "000000"
      rel_operator <- "000000"
      rel_passive_form <- "000000"
      rel_creation <- "000000"
      rel_destruction <- "000000"
      rel_causation <- "000000"
      rel_coref_res <- "000000"
      effect <- "000000"

    # print('noooooooooo')
  }
    return(list(id, cause, rel_negation,rel_operator, rel_passive_form, rel_creation, rel_destruction, rel_causation, rel_coref_res, effect))
